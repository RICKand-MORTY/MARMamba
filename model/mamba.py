...
class MambaBlock(nn.Module):
    ...
    def __init__(self, dim, bias=True, d_state=16, d_conv=4, expand=2, LayerNorm_type='WithBias'):
        ...
        self.attn = MambaAttention(dim, bias=False, d_state=16, d_conv=4, expand=2)
...
class MambaFormer(nn.Module):
    ...
    def __init__(
        ...
        self.down1_stage = Stage(dim=dim,depth=depth[0])
        self.down2_stage = Stage(dim=dim * 2 ** 1, depth=depth[1])
        self.down3_stage = Stage(dim=dim * 2 ** 2, depth=depth[2])
        self.down4_stage = Stage(dim=dim * 2 ** 3, depth=depth[3])
        self.up1_stage = Stage(dim=dim, depth=depth[0])
        self.up2_stage = Stage(dim=dim * 2 ** 1, depth= depth[1])
        self.up3_stage = Stage(dim=dim * 2 ** 2, depth= depth[2])
        ...
        if depth[4] != 0:
            self.refine_stage = Stage(dim=dim, depth=depth[4])
        else:

...
class ViTBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., LayerNorm_type='WithBias', bias=True):
        super().__init__()
        self.norm1 = LayerNorm(dim, LayerNorm_type)
        self.attn = ViTAttention(dim, qkv_bias=qkv_bias, num_heads=num_heads, attn_drop=attn_drop, proj_drop=proj_drop)
...
class Stage(nn.Module):
    ...
    def __init__(self,dim, depth, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., LayerNorm_type='WithBias'):
        super().__init__()
        ...
        self.blocks = nn.ModuleList([
            ViTBlock(dim=dim,qkv_bias=qkv_bias, num_heads=num_heads, attn_drop=attn_drop, proj_drop=proj_drop)
...
class ViTFormer(nn.Module):
    ...
    def __init__(
        ...
        self.down1_stage = Stage(dim=dim,depth=depth[0], num_heads=num_heads[0], qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=proj_drop, LayerNorm_type='WithBias')
        self.down2_stage = Stage(dim=dim * 2 ** 1, depth=depth[1], num_heads=num_heads[1], qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=proj_drop, LayerNorm_type='WithBias')
        self.down3_stage = Stage(dim=dim * 2 ** 2, depth=depth[2], num_heads=num_heads[2], qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=proj_drop, LayerNorm_type='WithBias')
        self.down4_stage = Stage(dim=dim * 2 ** 3, depth=depth[3], num_heads=num_heads[3], qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=proj_drop, LayerNorm_type='WithBias')
        self.up1_stage = Stage(dim=dim, depth=depth[0], num_heads=num_heads[2], qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=proj_drop, LayerNorm_type='WithBias')
        self.up2_stage = Stage(dim=dim * 2 ** 1, depth= depth[1], num_heads=num_heads[1], qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=proj_drop, LayerNorm_type='WithBias')
        self.up3_stage = Stage(dim=dim * 2 ** 2, depth= depth[2], num_heads=num_heads[0], qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=proj_drop, LayerNorm_type='WithBias')
        ...
        if depth[4] != 0:
            self.refine_stage = Stage(dim=dim, depth=depth[4], num_heads=num_heads[0], qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=proj_drop, LayerNorm_type='WithBias')
        else:
